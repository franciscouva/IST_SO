...
int ems_show(int out_fd, unsigned int event_id) {
  ...
  if (event_list == NULL) {
    fprintf(stderr, "EMS state must be initialized\n");
    if (write(out_fd, &returning, sizeof(int)) == -1) {
      perror("write");
    }
    return 1;
  }
  ...
  returning = 0;
  int rows = (int)event->rows;
  int cols = (int)event->cols;
  if (write(out_fd, &returning, sizeof(int)) == -1) {
    perror("write");
    return 1;
  }
  if (write(out_fd, &rows, sizeof(int)) == -1) {
    perror("write");
    return 1;
  }
  if (write(out_fd, &cols, sizeof(int)) == -1) {
    perror("write");
    return 1;
  }
  ...
  for (int i = 1; i <= rows; i++) {
    for (int j = 1; j <= cols; j++) {
      if (write(out_fd, &event->data[seat_index(event, (size_t)i, (size_t)j)], sizeof(unsigned int)) == -1) {
        perror("write");
      }
    }
  }
...
int ems_list_events(int out_fd) {
  ...
  if (current == NULL) {
    if ((write(out_fd, &returning, sizeof(int)) == -1) |
        (write(out_fd, &returning, sizeof(int)) == -1)) {
      perror("write");
    }
    pthread_rwlock_unlock(&event_list->rwl);
    return 0;
  ...
  size_t num_events = get_event_list_size();
  if (write(out_fd, &num_events, sizeof(size_t)) == -1) {
    perror("write");
  }

  while (1) {
    unsigned int value = (current->event)->id;
    if (write(out_fd, &value, sizeof(unsigned int)) == -1) {
      perror("write");
    }
